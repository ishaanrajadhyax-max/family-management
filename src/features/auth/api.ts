...
// True only while no admin account has credentials yet — see
// server/routes/auth.js. Drives whether the frontend shows the one-time
// "Set Up Admin Account" screen instead of the login form.
export async function needsSetup(): Promise<boolean> {
  const result = await authFetch<{ needsSetup: boolean }>('/auth/needs-setup')
  return result?.needsSetup ?? false
}

export async function setupAdmin(username: string, password: string): Promise<CurrentUser> {
  const user = await authFetch<CurrentUser>('/auth/setup-admin', {
    method: 'POST',
    body: JSON.stringify({ username, password }),
  })
  if (!user) {
    throw new Error('Could not set up the admin account.')
  }
  return user
}
